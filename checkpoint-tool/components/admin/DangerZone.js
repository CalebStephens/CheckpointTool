...
const [frustratedvsskills, setFrustratedvsSkills] = useState(true);
        ...
        hardVBoring.forEach(row => {
            hardTable.push(
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="px-2 py-2">{row["Lab ID"]}</td>
                    <td class="px-2 py-2">{row["Student Username"]}</td>
                    <td class="px-2 py-2">{row["X Value"]}</td>
                    <td class="px-2 py-2">{row["Y Value"]}</td>
                </tr>
            )
    ...
    <>
        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400" onClick={toggleHardTable}>
            <tr>
                <th class="px-6 py-3">
                    Lab ID
                </th>
        ...
        <tbody>
            {hardvsBoring ? renderHardTable() : null}
        </tbody>
        </table>

        <h2>I needed help to get started vs content was all new</h2>
        <table class="text-sm text-center text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400" onClick={toggleStartedvsContent}>
            <tr>
                <th class="px-6 py-3">
                    Lab ID
                </th>
        ...
        <tbody>
            {startedvscontent ? renderHardTable() : null}
        </tbody>
        </table>

        <h2>I feel frustrated VS my programming skills have improved</h2>
        <table class="text-sm text-center text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400" onClick={toggleFrustratedvsContent}>
            <tr>
                <th class="px-6 py-3">
                    Lab ID
                </th>
        ...
        <tbody>
            {frustratedvsskills ? renderHardTable() : null}
        </tbody>
        </table>
    </>
    )
